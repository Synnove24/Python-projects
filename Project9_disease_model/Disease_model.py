...
def population_SIR_counts(population:list)->dict:
    ...
    dict = {'infected':0,'susceptible':0,'recovered':0}
    for i in population:
        if i == "R":
            dict["recovered"] += 1
        elif i == "I":
...
def simulate_day(population:list,contact_range:int,infect_chance:float,recover_chance:float)->list:
...
#uses functions
counts = simulate_disease(5000,100,.99,.01)
